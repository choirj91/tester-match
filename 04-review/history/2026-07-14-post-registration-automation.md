...
| `GOOGLE_ADMIN_EMAIL` | 예: `admin@knockknock.company` | Domain-Wide Delegation impersonation 용 super admin |
| `TESTER_GROUP_EMAIL` | `testers@knockknock.company` | Workspace Admin 콘솔에서 생성한 그룹 이메일 |
| `TESTER_GROUP_URL` (선택) | `https://groups.google.com/a/knockknock.company/g/testers` | 미지정 시 이메일 기반 자동 추정 |

> **확보된 도메인**: `knockknock.company` (2026-07-14)

**Google Cloud / Workspace 설정 절차 (사용자 작업):**

1. **Cloud Identity Free 가입**: workspace.google.com/products/cloud-identity/free/
   - 도메인: `knockknock.company` (확보 완료)
   - DNS TXT 레코드로 도메인 인증
   - 최초 계정: `admin@knockknock.company` (super admin)
2. **그룹 생성**: Workspace Admin 콘솔 → 디렉터리 → 그룹 → 생성
   - 이메일: `testers@knockknock.company`
   - 액세스 설정: **조직 외부 사용자 가입 허용** (필수 — Tester Match 유저는 외부 gmail 계정)
3. **Google Cloud 프로젝트**:
