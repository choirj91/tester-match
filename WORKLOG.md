...
- **한 일**: ① CS — 앱 1561(YEONRI) 소유권 이전 (계정 분리 케이스, 참여·댓글 무손실) ② 관리자 통계 앱 순위 버그 수정 (apps에 deleted_at 없음→42703 침묵 실패 + PostgREST 1,000행 잘림 → fetchAll 페이지네이션) ③ 공개 활동 랭킹 `/stats` (전체 수치·방문자·랭킹 3종·크레딧 보상 예고 배너) + 공개 프로필 `/u/[id]` (등록 앱 리스트) + 네비 "랭킹"
- **결정**: 공개 페이지엔 이메일 등 개인정보 미노출 (닉네임·신뢰도만). 랭킹 크레딧 보상은 예고만 — 지급 설계는 추후
- **내일**: 랭킹 크레딧 보상 정책 설계 (지급 기준·주기·예산), 그룹 검증 잔여 항목
