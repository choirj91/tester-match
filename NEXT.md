...
- [ ] 게시판 스토리화 2단계 — 카톡 오픈채팅 반자동 다이제스트 (인기글 복붙용 카드 생성 → 관리자 발송)
- [ ] 게시판 스토리화 3단계 — 주간 글감 배너 ("이번 주: 첫 수익 인증")
- [ ] 이야기 시드 글감 — 리서치 프롬프트 결과로 초기 글 5편 작성·게시

- [ ] **knockknock.company 공식 홈페이지 신규 구축** — 멀티 제품 확장 구조 (회사 소개·제품 목록·소식·연락처). 별도 프로젝트. 요구사항: 루트 ads.txt 서빙(AdSense 소유권), admin@ 메일 표기. 임시 원페이지는 폐기함(b23bb05)

- [x] ~~랭킹 크레딧 보상 설계·구현~~ (2026-08-15 배포 — [정책 v1](03-output/policy/ranking-rewards.md), 9월분부터·10월 초 지급 버튼)
- [ ] 10/1~3 크론이 9월분 보상 자동 지급 — 결과만 /admin/ranking-rewards 에서 확인 (실패 시 수동 [지급])
- [x] ~~신뢰도 체크인 소급~~ (2026-08-15 — 시행 전 체크인당 +1 차액 일괄, 멱등 마이그레이션)
- [ ] 신뢰도 매칭 우선권 (v1.1) — 정원 경쟁 시 신뢰도 가중치
- [x] ~~신뢰도 정책 v1.1 + 구현~~ (2026-07-24 — ADR-0010, 체크인 +1·상한 1,000/옵트아웃 −3/이탈 −10)
- [x] ~~공개 활동 랭킹 + 유저 프로필~~ (2026-07-24 배포 — `/stats`, `/u/[id]`, 네비 "랭킹")
- [x] ~~개발자 테스터 모니터링 대시보드~~ (2026-07-22 배포 — 설치확인·출석·체크인·리마인드)
- [ ] 모니터링 실전 검증 — 본인 앱 + 부계정 2개로 E2E (설치확인 → 개발자 화면 반영 → 리마인드 수신)
...
| 날짜 | 작업 |
|---|---|
| 07-24 | 공개 활동 랭킹 `/stats` + 유저 프로필 `/u/[id]` (크레딧 보상 예고), 관리자 통계 앱 순위 버그 수정, YEONRI 소유권 이전 CS |
| 07-22 | 개발자 테스터 모니터링 (설치확인·출석·체크인·리마인드) |
